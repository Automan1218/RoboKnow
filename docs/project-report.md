...
**A proper controlled experiment was written and run on 2026-07-16** (`load-tests/scenario9-search-cache-latency.js`): same code, same query, fired twice back-to-back — first hit is a guaranteed Redis embedding-cache miss (equivalent to pre-`519226c` behavior, since a never-seen query always misses regardless of code version), second hit should be a guaranteed cache hit (post-`519226c` behavior). Result: **cold p95 = 4,098 ms, warm p95 = 4,023 ms — no meaningful difference**, because the same expired embedding API key (see Scenario 4) made *every* request fail all 3 retries and fall back to text-only search; since embeddings never succeeded, nothing was ever written to the Redis cache, so "warm" requests missed too. Both cold and warm are measuring the same retry-exhaustion backoff, not the cache. This confirms empirically — not just by inference — that **no valid hybrid-search-cache latency figure (1300 ms → 260 ms or otherwise) can be produced in this environment until the embedding API key is replaced.** Once it is, re-run `scenario9-search-cache-latency.js` for a real number.
