...
public class DocumentController {
    ...
    /**
     * 流式代理文件，强制 inline 展示（供 iframe 直接使用）
     */
    @GetMapping("/stream")
    public ResponseEntity<org.springframework.core.io.InputStreamResource> streamFile(
            @RequestParam String fileName,
            @RequestParam(required = false) String token,
            @RequestHeader(value = "Authorization", required = false) String authorizationHeader) {
        ...
        try {
            ...
            if (userId == null) {
                permitted = fileUploadRepository.findByFileNameAndIsPublicTrue(fileName).isPresent();
            } else {
                List<FileUpload> accessible = documentService.getAccessibleFiles(userId, orgTags);
                permitted = accessible.stream().anyMatch(f -> f.getFileName().equals(fileName));
            }
            ...
            java.io.InputStream stream = documentService.streamFile(fileName);
